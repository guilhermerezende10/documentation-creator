...
4. **Create the branch.** Build a slug from the issue title: lowercase, ASCII, hyphenated, max ~5 words, no leading/trailing hyphens. Branch name format: `issue/<number>-<slug>`. Run `git checkout -b issue/<number>-<slug>`.
5. **Plan.** Re-read the issue body in full. Identify:
...
6. **Implement** the smallest-change route the issue proposes (or your own equivalent if the proposal is wrong). Avoid drive-by refactors. Do not modify files the issue marks as out-of-scope.
7. **Verify.**
...
9. **Stop.** Do not commit, push, or open a PR unless the user explicitly asks. They may want to inspect the diff first.
