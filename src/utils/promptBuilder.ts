...
const SECTION_BRIEFS: Record<DocSectionId, SectionMeta> = {
  overview: {
    title: 'Overview',
    brief: 'What the project does, who it is for, and the problem it solves. 2 to 4 short paragraphs.',
  },
  installation: {
    title: 'Installation',
    brief: 'Prerequisites, install commands for the relevant package manager(s), and a verification step.',
  },
  usage: {
    title: 'Usage',
    brief: 'Minimal working example first, then 1 to 2 realistic patterns. Include CLI invocations or import snippets.',
  },
  api: {
    title: 'API Reference',
    brief: 'Public exports, signatures, parameter and return types, and meaningful errors. Group by module.',
  },
  deployment: {
    title: 'Deployment',
    brief: 'How to build, where it runs, required environment variables, and platform-specific notes.',
  },
  troubleshooting: {
    title: 'Troubleshooting',
    brief: 'Common errors with their causes and fixes. If unknown, leave a short "report issues at..." pointer.',
  },
};

export function listDocSections(): DocSectionId[] {
  return ['overview', 'installation', 'usage', 'api', 'deployment', 'troubleshooting'];
}
...
export function buildSectionPrompt(
  ...
  const meta = SECTION_BRIEFS[section];
  const answerLines = answers.length
    ? answers.map(a => `- ${a.questionId}: ${a.answer}`).join('\n')
    : '(no answers provided)';

  return [
    `You are a senior technical writer producing the "${meta.title}" section of project documentation.`,
    `Section brief: ${meta.brief}`,
    '',
    describeSource(input),
    '',
    'Clarification answers:',
    answerLines,
    '',
    `Output ONLY the markdown for this single section, starting with "## ${meta.title}".`,
    'No prose before or after, no whole-section code fence wrapping.',
  ].join('\n');
}
